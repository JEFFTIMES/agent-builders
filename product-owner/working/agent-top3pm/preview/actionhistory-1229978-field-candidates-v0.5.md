# ActionHistory Field Candidates - Mantis 1229978 (v0.5)
...
- each patch row carries:
  - `issueId` from `ID`
  - `mainBranchPointRaw` derived from linked `Issue.resolvedIn` token extraction
  - `issueTitle` sourced from linked `Issue.title`
...
If we use only `Issue` properties, we lose per-build patch context. `mainBranchPointRaw/mainBranchPoint` and build membership belong to the build-patch record, not globally to `Issue`.

## 4. Sample mapping
- `Top3Build | 8956 (for 3800G/3801G)` -> `Top3Build(buildNumber=8956, buildQualifier='for 3800G/3801G')`
- `Issue(1241309).resolvedIn = '2887'` -> `Patch(issueId=1241309, mainBranchPointRaw='2887', mainBranchPoint=2887, issueTitle=Issue(1241309).title)`
- `Issue(1225001).resolvedIn = 'Mantis'` -> `Patch(issueId=1225001, mainBranchPointRaw='Mantis', mainBranchPoint=null, issueTitle=Issue(1225001).title)`

## 5. Remaining confirmation
- `buildNumber` type: keep `string` (recommended) or force integer?
- tokenization rule for `Issue.resolvedIn`: agree parser spec (`first numeric token` fallback to normalized first token when no number)?
