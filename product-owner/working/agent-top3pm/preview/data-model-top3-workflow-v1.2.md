# Data Model - Top3 Workflow v1.2 (Entity-by-Entity, Discussion) - agent-top3pm
...
16. `ActionHistory.rawRef` stores source item ID.
17. `BranchRequest.NEW_BRANCH_NAME` normalizes to `Branch.branchName` when request succeeds.
18. `Branch.branchName` links Mantis/Phab/Jenkins contexts.
19. `BranchRequest.DEV_LEAD` / `BranchRequest.PM_LEAD` map to `Person.personId`.
20. `BuildRequestPage.pageUrl` is branch-page navigable endpoint.
21. `BuildRun.buildId/buildTag` links `Jenkins -> Outlook -> InfoSite`.
22. `Artifact.artifactPath` is InfoSite retrieval key.
23. `ValidationRecord.evidenceLink` links checklist/log evidence in Phabricator.
