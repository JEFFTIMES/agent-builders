# Data Model Discovery - top3-workflow v2.9 - agent-top3pm
...
- Source-of-Truth Model File: `working/agent-top3pm/preview/data-model-top3-workflow-v2.9.md`
- Graph Model File (canonical YAML): `working/agent-top3pm/preview/data-model-top3-workflow-graph-v2.9.yaml`
- Graph View File (Mermaid): `working/agent-top3pm/preview/data-model-top3-workflow-graph-v2.9.mmd`
...
## 1.1 External Data Touch Policy (First Round)
...
- `FOR_CUSTOMER` (boolean) [external_data_touch_policy=agent_managed] (should be )
...
- on the page, the agent mimics human user's operation, applying branch creation business rules, selecting a value from the dropdown options to fill the blank field for each property, or filling the text edit fields accordingly, then presses `build` button to run a build.
...
- internal data object, used to link BranchRequestPage, Email Notify and Branch
...
- `titleRaw` (test)
- `eventType` (enum: `top3case-bug-note`; `issue-bug-note`; `jenkins-branch-requested`; `jenkins-branch-approved`; `jenkins-branch-rejected`; `jenkins-building-success`; `jenkins-building-failure`; `phab-note`) (agent extracts from the titleRaw for)
...
- `buildNumber` (text nullable) (extract form the titleRaw when eventType = `jenkins-building-success`)
...
- `updatesStatus -> BranchRequest.requestStatus` (use eventType, key: branchName)


### Agent Actions

- Agent read email title and body to extract fields
- Agent updates `branchRequest` accordingly
...
- `createsBranch`(list[branch]:) #newly added, need to discuss how to fill the list, many sources exist, such as jenkins/user/<userId>/builds page, mantis branch-merge-list field, branch approved email, etc.
...
| ----------------------------------------------------- | ------------------------------------------------ | ----------------------------------------------- | ----------- |
| `BranchRequest` property/relation source checks       | source-check sections not yet documented in v2.9 | perform property + relation source-check review | Peter + HPO |
| `Branch` property/relation source checks              | source-check sections not yet documented in v2.9 | perform property + relation source-check review | Peter + HPO |
| `BuildRequestPage` property/relation source checks    | source-check sections not yet documented in v2.9 | perform property + relation source-check review | Peter + HPO |
| `BuildRequest` property/relation source checks        | source-check sections not yet documented in v2.9 | perform property + relation source-check review | Peter + HPO |
| `BuildRun` property/relation source checks            | source-check sections not yet documented in v2.9 | perform property + relation source-check review | Peter + HPO |
| `ValidationRecord` property/relation source checks    | source-check sections not yet documented in v2.9 | perform property + relation source-check review | Peter + HPO |
| `ReleaseNote` property/relation source checks         | source-check sections not yet documented in v2.9 | perform property + relation source-check review | Peter + HPO |
| `Person` property/relation source checks              | source-check sections not yet documented in v2.9 | perform property + relation source-check review | Peter + HPO |
| `Team` property/relation source checks                | source-check sections not yet documented in v2.9 | perform property + relation source-check review | Peter + HPO |
| `CommunicationThread` property/relation source checks | source-check sections not yet documented in v2.9 | perform property + relation source-check review | Peter + HPO |
