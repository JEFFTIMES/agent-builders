# Data Model Discovery - top3-workflow v3.2 - agent-top3pm
...
- Source-of-Truth Model File: `working/agent-top3pm/preview/data-model-top3-workflow-v3.2.md`
- Graph Model File (canonical YAML): `working/agent-top3pm/preview/data-model-top3-workflow-graph-v3.2.yaml`
- Graph View File (Mermaid): `working/agent-top3pm/preview/data-model-top3-workflow-graph-v3.2.mmd`
...
| BranchRequestPage   | Done           | Done           | Reviewed     | None                                  |
...
| Cross-artifact consistency | Markdown-only additions in prior version caused graph drift | Synchronize entity/relation additions across `v3.2.md`, `v3.2.yaml`, `v3.2.mmd` | Applied |
| Email notify extraction coverage | Marked samples had additional actor/request fields not normalized in entity map | Add `requestorId/devLeadId/pmLeadId` to `BranchRequest`, then map from email body and normalize to `Person.personId` | Applied |
...
- `branchRequestPageId` <- deterministic internal id derived from Jenkins request-form endpoint + product family context. `DESIGN FIELD`
- `pageUrl` <- `working/agent-top3pm/assets/top3apps/Jenkins-create-branch.html` (request form URL). `CONFIRMED`
- `GITLAB_TOP_LEVEL_GROUP` <- `working/agent-top3pm/assets/top3apps/Jenkins-create-branch.html` (Jenkins form parameter). `CONFIRMED`
- `GITLAB_PROJECT` <- `working/agent-top3pm/assets/top3apps/Jenkins-create-branch.html` (Jenkins form parameter). `CONFIRMED`
- `MAJOR_VERSION` <- `working/agent-top3pm/assets/top3apps/Jenkins-create-branch.html` (Jenkins form parameter). `CONFIRMED`
- `MINOR_VERSION` <- `working/agent-top3pm/assets/top3apps/Jenkins-create-branch.html` (Jenkins form parameter). `CONFIRMED`
- `NEW_BRANCH_NAME` <- `working/agent-top3pm/assets/top3apps/Jenkins-create-branch.html` (Jenkins form parameter). `CONFIRMED`
- `BRANCH_POINT` <- `working/agent-top3pm/assets/top3apps/Jenkins-create-branch.html` (Jenkins form parameter). `CONFIRMED`
- `BRANCH_TYPE` <- `working/agent-top3pm/assets/top3apps/Jenkins-create-branch.html` (Jenkins form parameter). `CONFIRMED`
- `BRANCH_DESCRIPTION` <- `working/agent-top3pm/assets/top3apps/Jenkins-create-branch.html` (Jenkins form parameter). `CONFIRMED`
- `FOR_CUSTOMER` <- `working/agent-top3pm/assets/top3apps/Jenkins-create-branch.html` (Jenkins form parameter). `CONFIRMED`
- `DEV_LEAD` <- `working/agent-top3pm/assets/top3apps/Jenkins-create-branch.html` (Jenkins form parameter). `CONFIRMED`
- `PM_LEAD` <- `working/agent-top3pm/assets/top3apps/Jenkins-create-branch.html` (Jenkins form parameter). `CONFIRMED`

#### Relation Source Check (Double-Checked)

- `submits -> BranchRequest` <- submit operation on Jenkins branch request form creates a request lifecycle tracked by request/approval emails. `CONFIRMED`
- `accessesBranchRequestPage <- Person(PM)` <- HPO workflow states PM is the standard role initiating branch requests via Jenkins form. `HPO-CONFIRMED INTERPRETATION`
...
- `requestorId` (personId, nullable) (normalized from `Requestor` in Branch Request notify)
- `devLeadId` (personId, nullable) (normalized from `Dev Lead` in Branch Request notify)
- `pmLeadId` (personId, nullable) (normalized from `PM Lead` in Branch Request notify)
...
- `requestedBy -> Person` (`0..1`) (from `requestorId`)
- `hasDevLead -> Person(Dev)` (`0..1`) (from `devLeadId`)
- `hasPmLead -> Person(PM)` (`0..1`) (from `pmLeadId`)
...
- `requestId` <- deterministic internal key (`branchName + requestedAt` hash fallback) to track a request across notifications. `DESIGN FIELD`
- `requestStatus` <- derived from related `EmailNotify.eventType` semantics (`requested/approved/rejected/...`). `HPO-CONFIRMED RULE`
- `requestedAt` <- branch request notify receive time in Outlook metadata. `CONFIRMED`
- `branchName` <- `working/agent-top3pm/assets/top3apps/branch-request-email-notify.txt` (`Branch Name` and title token). `CONFIRMED`
- `majorVersion` <- `working/agent-top3pm/assets/top3apps/branch-request-email-notify.txt` (`Version` major token). `CONFIRMED`
- `minorVersion` <- `working/agent-top3pm/assets/top3apps/branch-request-email-notify.txt` (`Version` minor token). `CONFIRMED`
- `branchPoint` <- `working/agent-top3pm/assets/top3apps/branch-request-email-notify.txt` (`Branch Point` token, e.g. `fg_7_2878` -> `2878`). `HPO-CONFIRMED RULE`
- `branchType` <- `working/agent-top3pm/assets/top3apps/branch-request-email-notify.txt` (`BranchType`). `CONFIRMED`
- `top3CaseId` <- `working/agent-top3pm/assets/top3apps/branch-request-email-notify.txt` (`Description`, mandatory Top3 case id entry). `HPO-CONFIRMED INTERPRETATION`
- `forCustomer` <- `working/agent-top3pm/assets/top3apps/branch-request-email-notify.txt` (`For Customer`). `CONFIRMED`
- `productFamily` <- `working/agent-top3pm/assets/top3apps/branch-request-email-notify.txt` (`Project`, e.g., FortiOS). `CONFIRMED`
- `requestorId` <- `working/agent-top3pm/assets/top3apps/branch-request-email-notify.txt` (`Requestor` account token). `CONFIRMED`
- `devLeadId` <- `working/agent-top3pm/assets/top3apps/branch-request-email-notify.txt` (`Dev Lead` account token). `CONFIRMED`
- `pmLeadId` <- `working/agent-top3pm/assets/top3apps/branch-request-email-notify.txt` (`PM Lead` account token). `CONFIRMED`
- `branchSourceCodeURL` <- `working/agent-top3pm/assets/top3apps/branch-approved-email-notify.txt` (`Source Code URL`, first URL only). `HPO-CONFIRMED RULE`
- `branchJobURL` <- `working/agent-top3pm/assets/top3apps/branch-approved-email-notify.txt` (`Jenkins Job URL`). `CONFIRMED`

#### Relation Source Check (Double-Checked)

- `generatesBranch -> Branch` <- successful request approval yields one canonical branch name; maps request payload to `Branch.branchName`. `CONFIRMED`
- `raisedForTop3Case -> Top3Case` <- `top3CaseId` is carried via request description and links request to a single case context. `HPO-CONFIRMED INTERPRETATION`
- `requestedBy -> Person` <- resolve `requestorId` to `Person.personId` from request notify payload. `CONFIRMED`
- `hasDevLead -> Person(Dev)` <- resolve `devLeadId` to `Person.personId` from request notify payload. `CONFIRMED`
- `hasPmLead -> Person(PM)` <- resolve `pmLeadId` to `Person.personId` from request notify payload. `CONFIRMED`
- `submits <- BranchRequestPage` <- request is initiated by Jenkins request-page submit action. `CONFIRMED`
- `updatesStatus <- EmailNotify` <- later branch-related notifies update lifecycle status and enrich links (`approved/rejected/job-url`). `CONFIRMED`
...
- `emailNotifyId` <- deterministic message identity from mail headers (`message-id`) or normalized subject+receivedAt fallback. `DESIGN FIELD`
- `receivesAt` <- Outlook message receive timestamp (`Received` metadata). `CONFIRMED`
- `sender` <- Outlook sender address in mail header (e.g., Jenkins/Mantis notifiers). `CONFIRMED`
- `titleRaw` <- raw email subject/title in Outlook mail item. `CONFIRMED`
- `eventType` <- derived classifier from `titleRaw` + body patterns (`branch request`, `branch approved`, `build success/failure`, `mantis note`, `phab note`). `HPO-CONFIRMED RULE`
- `branchName` <- extracted from title/body tokens in branch-related notifications (`Branch <...>`). `CONFIRMED`
- `branchSourceCodeURL` <- `working/agent-top3pm/assets/top3apps/branch-approved-email-notify.txt` (`Source Code URL`, first URL only). `HPO-CONFIRMED RULE`
- `branchJobURL` <- `working/agent-top3pm/assets/top3apps/branch-approved-email-notify.txt` (`Jenkins Job URL`). `CONFIRMED`
- `buildNumber` <- extracted from build-notify subject/body tokens when present (`Build x.y.z.<num>`). `HPO-CONFIRMED INTERPRETATION`
- `imagesStoreAt` <- extracted from build success notify title/body when image storage location is present. `HPO-CONFIRMED INTERPRETATION`
- `consoleOutputURL` <- extracted from build failure notify body (`Console Output` URL). `HPO-CONFIRMED INTERPRETATION`
- `ticketURL` <- extracted from Mantis/Phabricator update notifies when ticket link exists. `CONFIRMED`

#### Relation Source Check (Double-Checked)

- `updatesStatus -> BranchRequest` <- branch-related notifications are matched by `branchName` and event semantics to update request lifecycle and enrich branch artifacts. `CONFIRMED`
...
33. `BranchRequest.requestorId` / `devLeadId` / `pmLeadId` are normalized to `Person.personId` from branch-request notify payload.
34. `BranchRequest.top3CaseId` links to `Top3Case.caseId` via request `Description`.
35. `EmailNotify.eventType` is derived from email title/body and drives branch-request status sync.
36. `EmailNotify.branchName` is the join key to `BranchRequest.branchName` for status updates.
37. `BuildRequestPage.pageUrl` is branch-page navigable endpoint.
38. `BuildRun.buildId/buildTag` links `Jenkins -> Outlook -> InfoSite`.
39. `Top3Build.buildNumber` is the operational retrieval key for build artifacts in InfoSite context.
40. `ValidationRecord.evidenceLink` links checklist/log evidence in Phabricator.
41. `Team.channelRef` and `CommunicationThread.threadId` identify Core/All channels.
42. `Issue.hasBugNote -> BugNote` is the canonical issue-note linkage keyed by issue `bug_id` and bugnote IDs.
43. `Issue.referredIssueIds` are agent-extracted issue IDs from linked issue bugnote text (`Issue.hasBugNote -> BugNote.rawText`) using `#id`, `mantis#id`, `bug_id=id`.
44. `Issue.referredNfrIds` are agent-extracted NFR IDs from linked issue bugnote text (`Issue.hasBugNote -> BugNote.rawText`) using `#id`, `mantis#id`, `bug_id=id`.
45. `Issue.references -> Issue` resolves from `Issue.referredIssueIds` after ticket-type filtering.
46. `Issue.references -> NFR` resolves from `Issue.referredNfrIds` after ticket-type filtering.
47. `Issue.duplicateIssueId` maps from Mantis `Duplicate ID`; `Issue.duplicateOf -> Issue` resolves when value is present.
48. `Issue.assignedDevId` maps from Mantis `Assigned To` and normalizes to `Person.personId` account token.
49. `Issue.qaAssigneeId` maps from Mantis `QA Assignee` and normalizes to `Person.personId` account token.
50. `Issue.fixSchedule` and `Issue.resolvedIn` map directly from same-name Mantis fields.
51. `Issue.category`, `Issue.severity`, `Issue.reproducibility`, `Issue.reporterId`, `Issue.keyword`, `Issue.reproducedById`, `Issue.reportedVersion`, and `Issue.earliestBuildNumber` map directly from same-name Mantis issue fields (`Reporter`/`Reproduced By` normalize to `Person.personId`).
52. `Issue.conclusion` is AI-agent inferred/composed from full issue-ticket evidence and stored as synthesized closure narrative.
53. `Issue.assignedDevId`/`Issue.qaAssigneeId` are denormalized snapshots; `Issue.fixedBy`/`Issue.hasQaAssignee` relations are canonical for graph traversal.
54. `PhabricatorTask.assignedToId` is denormalized snapshot; `PhabricatorTask.assignedTo` relation is canonical.
...
| ----------------------------------------------------- | ------------------------------------------------ | ----------------------------------------------- | ----------- |
| `Branch` property/relation source checks              | source-check sections not yet documented in v3.2 | perform property + relation source-check review | Peter + HPO |
| `BuildRequestPage` property/relation source checks    | source-check sections not yet documented in v3.2 | perform property + relation source-check review | Peter + HPO |
| `BuildRequest` property/relation source checks        | source-check sections not yet documented in v3.2 | perform property + relation source-check review | Peter + HPO |
| `BuildRun` property/relation source checks            | source-check sections not yet documented in v3.2 | perform property + relation source-check review | Peter + HPO |
| `ValidationRecord` property/relation source checks    | source-check sections not yet documented in v3.2 | perform property + relation source-check review | Peter + HPO |
| `ReleaseNote` property/relation source checks         | source-check sections not yet documented in v3.2 | perform property + relation source-check review | Peter + HPO |
| `Person` property/relation source checks              | source-check sections not yet documented in v3.2 | perform property + relation source-check review | Peter + HPO |
| `Team` property/relation source checks                | source-check sections not yet documented in v3.2 | perform property + relation source-check review | Peter + HPO |
| `CommunicationThread` property/relation source checks | source-check sections not yet documented in v3.2 | perform property + relation source-check review | Peter + HPO |
...
- [2026-02-26 10:42:10 PST] `v3.2` branch-request/email normalization: completed source-check sections for `BranchRequestPage`, `BranchRequest`, `EmailNotify`; added `requestorId/devLeadId/pmLeadId`; and aligned upstream/downstream relation mappings.
