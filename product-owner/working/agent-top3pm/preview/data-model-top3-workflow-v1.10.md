# Data Model Discovery - top3-workflow v1.10 - agent-top3pm
...
- Source-of-Truth Model File: `working/agent-top3pm/preview/data-model-top3-workflow-v1.10.md`
- Graph Model File (canonical YAML): `working/agent-top3pm/preview/data-model-top3-workflow-graph-v1.10.yaml`
- Graph View File (Mermaid): `working/agent-top3pm/preview/data-model-top3-workflow-graph-v1.10.mmd`
...
6. `Issue.issueId` = bug-fix `Mantis bug_id`.
7. `BugNote.bugNoteId` maps to Mantis bugnote ID when available.
8. `BugNote.mentionedMantisIds` stores all `#<mantisId>` mentions.
9. `BugNote.mentionedTicketIds` are parsed Mantis cross-record references.
10. `BugNote.phabTaskIds` are resolved from Phabricator search using `Top3Case.caseId` + QA identity context.
11. `BugNote.bugNoteEditUrl`/`bugNoteDeleteUrl` are agent shortcut URLs derived from ticket and bugnote ids.
12. `Person.personId` should use unique company account id (e.g., `sjeff`) when mapping `BugNote authoredBy Person`.
13. `PhabricatorTask.phabTaskId` is the canonical cross-system key (e.g., `T28209`).
14. `PhabricatorTask.top3CaseId` links to `Top3Case.caseId` via Phab `Mantis IDs` mapping.
15. `Top3Case.currentPhase` may be derived from `BugNote.headlineType` plus note content cues, with provenance in `currentPhaseSource/currentPhaseEvidenceNoteId`.
16. Release summary content is captured in `BugNote(headlineType=release)`; no standalone Top3Case release-summary property.
17. `Top3Case.closureReason` is AI-agent inferred/composed from final state and close/release bugnote evidence.
18. `ActionHistory.rawRef` stores source item ID.
19. `BranchRequest.NEW_BRANCH_NAME` normalizes to `Branch.branchName` when request succeeds.
20. `Branch.branchName` links Mantis/Phab/Jenkins contexts.
21. `BranchRequest.DEV_LEAD` / `BranchRequest.PM_LEAD` map to `Person.personId`.
22. `BuildRequestPage.pageUrl` is branch-page navigable endpoint.
23. `BuildRun.buildId/buildTag` links `Jenkins -> Outlook -> InfoSite`.
24. `Artifact.artifactPath` is InfoSite retrieval key.
25. `ValidationRecord.evidenceLink` links checklist/log evidence in Phabricator.
26. `Team.channelRef` and `CommunicationThread.threadId` identify Core/All channels.
27. `Issue.hasBugNote -> BugNote` is the canonical issue-note linkage keyed by issue `bug_id` and bugnote IDs.
28. `Issue.referredIssueIds` are agent-extracted issue IDs from linked issue bugnote text (`Issue.hasBugNote -> BugNote.rawText`) using `#id`, `mantis#id`, `bug_id=id`.
29. `Issue.linksTo -> Issue` resolves from `Issue.referredIssueIds` after ticket-type filtering.
30. `Issue.duplicateIssueId` maps from Mantis `Duplicate ID`; `Issue.duplicateOf -> Issue` resolves when value is present.
31. `Issue.assignedDevId` maps from Mantis `Assigned To` and normalizes to `Person.personId` account token.
32. `Issue.qaAssigneeId` maps from Mantis `QA Assignee` and normalizes to `Person.personId` account token.
33. `Issue.fixSchedule` and `Issue.resolvedIn` map directly from same-name Mantis fields.
34. `Issue.category`, `Issue.severity`, `Issue.reproducibility`, `Issue.reporterId`, `Issue.keyword`, `Issue.reproducedById`, `Issue.reportedVersion`, and `Issue.earliestBuildNumber` map directly from same-name Mantis issue fields (`Reporter`/`Reproduced By` normalize to `Person.personId`).
35. `Issue.conclusion` is AI-agent inferred/composed from full issue-ticket evidence and stored as synthesized closure narrative.
36. `Issue.assignedDevId`/`Issue.qaAssigneeId` are denormalized snapshots; `Issue.fixedBy`/`Issue.hasQaAssignee` relations are canonical for graph traversal.
37. `PhabricatorTask.assignedToId` is denormalized snapshot; `PhabricatorTask.assignedTo` relation is canonical.
...
| ----------------------------------------------------- | ------------------------------------------------ | ----------------------------------------------- | ----------- |
| `ActionHistory` property/relation source checks       | source-check sections not yet documented in v1.10 | perform property + relation source-check review | Peter + HPO |
| `BranchRequest` property/relation source checks       | source-check sections not yet documented in v1.10 | perform property + relation source-check review | Peter + HPO |
| `Branch` property/relation source checks              | source-check sections not yet documented in v1.10 | perform property + relation source-check review | Peter + HPO |
| `BuildRequestPage` property/relation source checks    | source-check sections not yet documented in v1.10 | perform property + relation source-check review | Peter + HPO |
| `BuildRequest` property/relation source checks        | source-check sections not yet documented in v1.10 | perform property + relation source-check review | Peter + HPO |
| `BuildRun` property/relation source checks            | source-check sections not yet documented in v1.10 | perform property + relation source-check review | Peter + HPO |
| `Artifact` property/relation source checks            | source-check sections not yet documented in v1.10 | perform property + relation source-check review | Peter + HPO |
| `ValidationRecord` property/relation source checks    | source-check sections not yet documented in v1.10 | perform property + relation source-check review | Peter + HPO |
| `ReleaseNote` property/relation source checks         | source-check sections not yet documented in v1.10 | perform property + relation source-check review | Peter + HPO |
| `Person` property/relation source checks              | source-check sections not yet documented in v1.10 | perform property + relation source-check review | Peter + HPO |
| `Team` property/relation source checks                | source-check sections not yet documented in v1.10 | perform property + relation source-check review | Peter + HPO |
| `CommunicationThread` property/relation source checks | source-check sections not yet documented in v1.10 | perform property + relation source-check review | Peter + HPO |
