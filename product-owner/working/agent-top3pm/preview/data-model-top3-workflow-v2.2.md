# Data Model Discovery - top3-workflow v2.2 - agent-top3pm
...
- Source-of-Truth Model File: `working/agent-top3pm/preview/data-model-top3-workflow-v2.2.md`
- Graph Model File (canonical YAML): `working/agent-top3pm/preview/data-model-top3-workflow-graph-v2.2.yaml`
- Graph View File (Mermaid): `working/agent-top3pm/preview/data-model-top3-workflow-graph-v2.2.mmd`
...
- `solution` (string, from Mantis `Solution` field; comma-separated tags)
...
- `actionHistoryId` (string, primary key, deterministic key by `caseId + sequenceNo + rowType + mantisId/buildNumber`)
- `caseId` (string, parent Top3 case id)
- `sequenceNo` (integer, row order in Action History section)
- `rowType` (enum: `build_marker`/`fix_entry`/`spacer`)
- `buildNumber` (string, current Top3Build context inherited by following fix rows until next Top3Build row)
- `buildQualifier` (text, nullable; build suffix note such as `for 3800G/3801G`)
- `mantisId` (string, nullable; linked ticket id in fix row)
- `mergeTargetRaw` (string, nullable; raw value in `Merged` column, e.g., `1978`/`2887`/`Mantis`)
...
- `summary` (text, nullable; fix row summary text)
- `sourceSystem` (enum: Mantis)
- `sourceSection` (string, default: `Action History`)
- `sourceTicketId` (string, Mantis ticket id where this section is scraped)
- `rawIdCell` (text)
- `rawMergedCell` (text)
- `rawSummaryCell` (text)
...
- `references -> Issue` (`0..1`, when `mantisId` resolves to issue ticket type by lookup)
- `references -> NFR` (`0..1`, when `mantisId` resolves to NFR ticket type by lookup)

#### Property Source Check (Double-Checked)

- `caseId` <- Mantis page context (`bug_id`) of parent Top3 ticket. `CONFIRMED`
- `sourceTicketId` <- Mantis page context (`bug_id`) where `Action History` section is extracted. `CONFIRMED`
- `sequenceNo` <- row order from `Action History` table traversal. `CONFIRMED`
- `rowType` <- row pattern in `Action History` table (`Top3Build` marker row / linked ticket row / spacer row). `CONFIRMED`
- `buildNumber` <- `Action History` build-marker row: first cell `Top3Build`, second cell value (e.g., `8928`, `8956 (for 3800G/3801G)`) parsed to numeric tag text. `HPO-CONFIRMED INTERPRETATION`
- `buildQualifier` <- suffix text in build-marker value (e.g., `for 3800G/3801G`) when present. `HPO-CONFIRMED INTERPRETATION`
- `mantisId` <- linked fix row first-column anchor text/link target `bug_id`. `CONFIRMED`
- `mergeTargetRaw` <- linked fix row second-column `Merged` cell raw text (numeric branchpoint or non-numeric token such as `Mantis`). `HPO-CONFIRMED INTERPRETATION`
- `summary` <- linked fix row third-column `Summary` cell text. `CONFIRMED`
- `mergedToTrunk` <- derived from `mergeTargetRaw`: numeric -> true; otherwise false. `HPO-CONFIRMED RULE`
- `trunkBranchPoint` <- derived numeric parse of `mergeTargetRaw` when present; null otherwise. `HPO-CONFIRMED RULE`
...
- `rawIdCell`/`rawMergedCell`/`rawSummaryCell` <- raw HTML cell text captured from each Action History row. `CONFIRMED`

#### Relation Source Check (Double-Checked)

- `hasActionHistory <- Top3Case` <- `Action History` section in Mantis case page keyed by parent `bug_id`. `CONFIRMED`
- `references -> Issue` <- `mantisId` resolved by agent lookup to issue-type ticket. `HPO-CONFIRMED INTERPRETATION`
- `references -> NFR` <- `mantisId` resolved by agent lookup to NFR-type ticket. `HPO-CONFIRMED INTERPRETATION`
...
19. `ActionHistory.actionHistoryId` is deterministic by `caseId + sequenceNo + rowType + mantisId/buildNumber`.
20. `ActionHistory` uses grouped-build semantics: each `Top3Build` marker defines `buildNumber` context for following fix rows until the next marker.
21. `ActionHistory.mergeTargetRaw` stores raw `Merged` cell text; numeric values represent trunk branchpoint.
22. `ActionHistory.mergedToTrunk`/`trunkBranchPoint` are derived by agent from `mergeTargetRaw` parsing rule (numeric vs non-numeric).
23. `BranchRequest.NEW_BRANCH_NAME` normalizes to `Branch.branchName` when request succeeds.
24. `Branch.branchName` links Mantis/Phab/Jenkins contexts.
25. `BranchRequest.DEV_LEAD` / `BranchRequest.PM_LEAD` map to `Person.personId`.
26. `BuildRequestPage.pageUrl` is branch-page navigable endpoint.
27. `BuildRun.buildId/buildTag` links `Jenkins -> Outlook -> InfoSite`.
28. `Artifact.artifactPath` is InfoSite retrieval key.
29. `ValidationRecord.evidenceLink` links checklist/log evidence in Phabricator.
30. `Team.channelRef` and `CommunicationThread.threadId` identify Core/All channels.
31. `Issue.hasBugNote -> BugNote` is the canonical issue-note linkage keyed by issue `bug_id` and bugnote IDs.
32. `Issue.referredIssueIds` are agent-extracted issue IDs from linked issue bugnote text (`Issue.hasBugNote -> BugNote.rawText`) using `#id`, `mantis#id`, `bug_id=id`.
33. `Issue.linksTo -> Issue` resolves from `Issue.referredIssueIds` after ticket-type filtering.
34. `Issue.duplicateIssueId` maps from Mantis `Duplicate ID`; `Issue.duplicateOf -> Issue` resolves when value is present.
35. `Issue.assignedDevId` maps from Mantis `Assigned To` and normalizes to `Person.personId` account token.
36. `Issue.qaAssigneeId` maps from Mantis `QA Assignee` and normalizes to `Person.personId` account token.
37. `Issue.fixSchedule` and `Issue.resolvedIn` map directly from same-name Mantis fields.
38. `Issue.category`, `Issue.severity`, `Issue.reproducibility`, `Issue.reporterId`, `Issue.keyword`, `Issue.reproducedById`, `Issue.reportedVersion`, and `Issue.earliestBuildNumber` map directly from same-name Mantis issue fields (`Reporter`/`Reproduced By` normalize to `Person.personId`).
39. `Issue.conclusion` is AI-agent inferred/composed from full issue-ticket evidence and stored as synthesized closure narrative.
40. `Issue.assignedDevId`/`Issue.qaAssigneeId` are denormalized snapshots; `Issue.fixedBy`/`Issue.hasQaAssignee` relations are canonical for graph traversal.
41. `PhabricatorTask.assignedToId` is denormalized snapshot; `PhabricatorTask.assignedTo` relation is canonical.
...
| `ActionHistory.mergedToTrunk`         | derive trunk-merge boolean from `Merged` column token                  | `ActionHistory.mergeTargetRaw`                                                                           | parse and set merge flag (`numeric=true`, non-numeric=`false`) | `HPO-CONFIRMED RULE`         |
| `ActionHistory.trunkBranchPoint`      | derive numeric trunk branchpoint from `Merged` column when available   | `ActionHistory.mergeTargetRaw`                                                                           | parse integer branchpoint or null                               | `HPO-CONFIRMED RULE`         |
...
| ----------------------------------------------------- | ------------------------------------------------ | ----------------------------------------------- | ----------- |
| `BranchRequest` property/relation source checks       | source-check sections not yet documented in v2.2 | perform property + relation source-check review | Peter + HPO |
| `Branch` property/relation source checks              | source-check sections not yet documented in v2.2 | perform property + relation source-check review | Peter + HPO |
| `BuildRequestPage` property/relation source checks    | source-check sections not yet documented in v2.2 | perform property + relation source-check review | Peter + HPO |
| `BuildRequest` property/relation source checks        | source-check sections not yet documented in v2.2 | perform property + relation source-check review | Peter + HPO |
| `BuildRun` property/relation source checks            | source-check sections not yet documented in v2.2 | perform property + relation source-check review | Peter + HPO |
| `Artifact` property/relation source checks            | source-check sections not yet documented in v2.2 | perform property + relation source-check review | Peter + HPO |
| `ValidationRecord` property/relation source checks    | source-check sections not yet documented in v2.2 | perform property + relation source-check review | Peter + HPO |
| `ReleaseNote` property/relation source checks         | source-check sections not yet documented in v2.2 | perform property + relation source-check review | Peter + HPO |
| `Person` property/relation source checks              | source-check sections not yet documented in v2.2 | perform property + relation source-check review | Peter + HPO |
| `Team` property/relation source checks                | source-check sections not yet documented in v2.2 | perform property + relation source-check review | Peter + HPO |
| `CommunicationThread` property/relation source checks | source-check sections not yet documented in v2.2 | perform property + relation source-check review | Peter + HPO |
