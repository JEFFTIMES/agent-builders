...
6. Outgoing and incoming relations in entity cards are two views of the same canonical relation and are for documentation/navigation clarity.
7. In graph artifacts (`.yaml` and `.mmd`), declare each canonical relation once only; do not add a second reverse edge solely to mirror incoming view.
8. Add a second edge only when the semantics are different (different relation type, rule, ownership, or lifecycle meaning), not just opposite reading direction.
...
9. Relation duplication check passes: incoming/outgoing documentation may be bidirectional, but graph edges remain single-canonical unless semantics differ.
